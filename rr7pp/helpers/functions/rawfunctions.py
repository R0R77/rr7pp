...
class RawFunctions(object):
	...
	async def error(
		...
		teks = f"**تقرير خطأ المشكلة:**\n\n"
		teks += f"**الوقت:** `{self.showdate()}`\n**Time:** `{self.showtime()}`\n\n"
		teks += f"**يمكنك اعادة توجيه هذه الرسالة الى مجموعة جمثون للتعرف على الحل @jmthon_support**\n\n" 
		teks += f"**الأمر:** `{message.text}`\n\n"
		teks += "`-`" * 30 + "\n\n"
		teks += f"**الخطأ:** \n\n`{e}`\n\n"
	...
	async def check_private(
		...
		if message.chat.type == "private":
			await self.send_edit(
				message, 
				"**- عذرا هذا الامر يستخدم فقط في الكروبات** . . .",
				text_type=["mono"], 
				delme=4
			)
			return True
		...
		try:
			...
			if send:
				await self.send_document(
					message.chat.id,
					path,
					caption = caption if caption else f"**تم الرفع بواسطة:** {self.UserMention()}"
				)
	...
	async def toggle_inline(
		...
		try:
			botname = "BotFather"
			await self.send_edit(message, "**- انتظر قليلا تتم العملية** . . .", text_type=["mono"])
			await self.send_message(botname, "/mybots") # BotFather (93372553) 
			await asyncio.sleep(0.50) # فلود ويت
			...
			await self.send_edit(message, "**- جار اختيار البوت  .  .**", text_type=["mono"])

			if self.bot.username in unames:
				await data[0].click(self.bot.username)
			else:
				return await self.send_edit(message, "- يبدو انه ليس لديك بوت يجب عليك صنع بوت بنفسك .", text_type=["mono"], delme=4)

			data = await self.get_last_msg(message, botname)

			await self.send_edit(message, "- يتم اختيار الاعدادات . . . ", text_type=["mono"])

			await data[0].click("Bot Settings")

			data = await self.get_last_msg(message, botname)

			await self.send_edit(message, "**- يتم التأكد من حالة الانلاين في البوت** ", text_type=["mono"])
			...
			if "Turn on" in str(data[0]):
				await self.send_edit(message, "**- جار تفعيل وضع الانلاين في البوت** ", text_type=["mono"])
				await data[0].click("Turn on")
				await self.send_edit(message, "** تم بنجـاح تفـعيل وضـع الأنـلاين**.", text_type=["mono"], delme=4)
			# Turn inline mode off
			elif "Turn inline mode off" in str(data[0]):
				await self.send_edit(message, "جار تعطيل وضع الانلاين في البوت", text_type=["mono"])
				await data[0].click("Turn inline mode off")
				await self.send_edit(message, " تم بنجـاح تعطـيل وضـع الأنـلاين.", text_type=["mono"], delme=4)
	...
	def import_module(
		...
		ex: 
			app.import_module("./rr7pp/modules/", exclude=["admin"])
		"""
