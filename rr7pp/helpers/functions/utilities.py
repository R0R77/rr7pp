...
class Utilities(AioHttp):
		...
		if len(twins) > rows:
			twins = twins[
				...
				(
					...
					InlineKeyboardButton(text="• رجوع •", callback_data=f"open-start-dex"),
					InlineKeyboardButton(
						text="التالي",
	...
	def TimeParser(self, start, end=None) -> int:
		...
		times = ""
		if month:
			times += "{} شهر، ".format(month)
		if days:
			times += "{} يوم، ".format(days)
		if hours:
			times += "{} ساعة، ".format(hours)
		if minutes:
			times += "{} دقيقة، ".format(minutes)
		if seconds:
			times += "{} ثانية".format(seconds)
		if times == "":
			times = "{} ميلي ثانية".format(time_end)
		return times
